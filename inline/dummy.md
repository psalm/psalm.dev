...
Psalm now supports `key-of<...>` and `value-of<...>`. These types allow you to restrict expected input and output based on class constant arrays:
...
`class-string<SomeClass>` is more specific – it will only accept class-strings that represent the object in question, or one of its descendants. It means we can write a certain sort of code that Psalm can verify:
...
Psalm 2 introduced Psalter, a tool that allows you to fix some of the errors Psalm finds. It now has a few extra features:
...
None of those tools quite worked with code annotated in the comprehensive style Psalm encourages, so [I’ve added that functionality to Psalm](https://psalm.dev/docs/manipulating_code/refactoring/).
...
Taint analysis is a cousin of type analysis - instead of keeping track a variable’s type, we record whether it’s “tainted” - i.e. if the contents of the variable could be controlled by an attacker, and could leak somehow into operations that assume their inputs are well-formed.

Taint analysis can typically follow the taintedness of a given variable across function boundaries, and can mark a variable as safe if it passes through particular configured functions. Taint analysis relies on sources and sinks - sources are things that could be controlled by an attacker e.g. `$id = $_GET['id']` and sinks are places where we don’t want tainted data (e.g. `<div id="t_<?= $id ?>">`).

The basic mechanics of Psalm’s implementation are working, and it has already discovered a few XSS vulnerabilities in Vimeo’s codebase, but there’s still more to do - I’m hoping to release the feature properly in late September/early October. [Follow Psalm on Twitter](https://twitter.com/psalmphp) for updates!
